...
function Menza() {
  ...
  const closeRatingForm = async () => {
    ...
    try {
      ...
      const [hrana, ljubaznost, ambijent, lokacija] = response.data; // Destructure the array
      setOcjene({ hrana, ljubaznost, ambijent, lokacija });
      console.log("Ocjene uspješno dohvaćene nakon zatvaranja popup-a.");
    } catch (error) {
      console.error(
        "Greška prilikom dohvaćanja ocjena nakon zatvaranja:",
        error
      );
    }
  };
  ...
  const sortRadnaVremena = (
    formattedRadnaVremena: RestaurantData["radnaVremena"]
  ) => {
    const daysOrder = [
      "Ponedjeljak",
      "Utorak",
    ...
    formattedRadnaVremena.sort(
      (a, b) => daysOrder.indexOf(a.dan) - daysOrder.indexOf(b.dan)
    );

    return formattedRadnaVremena;
  };
  ...
  useEffect(() => {
    ...
    sortRadnaVremena(formattedRadnaVremena);
  ...
  const submitRating = async () => {
    ...
    const payload = {
      hranaRating: rating.hrana,
      ljubaznostRating: rating.ljubaznost,
      ambijentRating: rating.ambijent,
      lokacijaRating: rating.lokacija,
    };
