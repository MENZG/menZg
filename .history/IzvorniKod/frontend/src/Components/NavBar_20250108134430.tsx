    ...
    const checkUserStatus = async () => {
      try {
        ...
        if (responseUser.status === 200) {
          setIsLoggedIn(true);
          if(responseKorisnici.data username === responseUser.data.email){
            if responseKorisnici.data.role === 1
            setRole("Student")
            if responseKorisnici.data.role === 2
            setRole("Djelatnik menze")
            if responseKorisnici.data.role === 3
