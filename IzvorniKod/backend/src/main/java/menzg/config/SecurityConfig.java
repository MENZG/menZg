...
public class SecurityConfig {
        ...
        return http.csrf(csrf -> csrf
           ...
           /* auth.requestMatchers("/h2-console/**").permitAll();*/
            auth.anyRequest().authenticated();
                ...
                // .headers(headers -> headers.frameOptions().sameOrigin())//.frameOptions().sameOrigin()
                .oauth2Login(oauth2 -> {
                    oauth2
                            .userInfoEndpoint(
   ...
   /* oauth2 -> {
    ...
    //objasnjenje drugog
    /*Konfigurira OAuth2 autentifikaciju kada se korisnik prijavljuje putem OAuth2 protokola.

    userInfoEndpoint(userInfoEndpoint -> userInfoEndpoint.userAuthoritiesMapper(this.authorityMapper())):
    Postavlja mapiranje korisničkih prava (userAuthoritiesMapper). Metoda authorityMapper() je metoda iz trenutne klase koja prilagođava prava korisnika prema informacijama koje pruža OAuth2 provajder (npr. Google).

    successHandler((request, response, authentication) -> { ... }):
    Definira ponašanje nakon uspješne autentifikacije. U ovom slučaju, nakon uspješne prijave korisnik se preusmjerava na određeni frontendUrl.*/
